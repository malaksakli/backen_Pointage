...
public class PointageController {
    ...
    @PostMapping("/ajouter")
    public ResponseEntity<String> ajouterPointage(@RequestBody Pointage pointage, @RequestParam Long matr) {
        try {
            // Vérifier si l'employé existe
            Employee employee = employeeRepository.findByMatr(matr).get();
            if (employee == null) {
                return ResponseEntity.badRequest()
                        .body("Erreur : L'employé avec le matricule " + matr + " n'existe pas.");
            }

            // Associer l'employé au pointage
            pointage.setEmployee(employee);

            // Valider les états du pointage
            ...
            // Ajouter le pointage
            Pointage savedPointage = pointageService.ajouterPointage(pointage,matr);
    ...
    public  List<Map<String, Object>> calculatePointage(
            @RequestParam int year,
            @RequestParam String states) {

        List<Map<String, Object>> result = pointageService.getRecapitulatifParServiceEtMois(year, states);
        return result;
    }
...
//                pointage.setDate(today);
    ...
    public Pointage modifier(@PathVariable Long id, @RequestBody Pointage pointageDetails) {
     return  pointageService.modifier(id,pointageDetails);
    }
 // Endpoint pour récupérer les heures travaillées d'un employé
    ...
    public Pointage getbyid(@PathVariable Long id) {
      return pointageService.getbyid(id);
    }
    @GetMapping("/getbymatr/{id}")
    public List<Pointage> getbymatr(@PathVariable Long id) {
      return pointageService.getPointagesbymatr(id);
    }
